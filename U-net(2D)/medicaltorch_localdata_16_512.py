...
for i in range(120):
    for j in [0,1,2]:
        if str(i)+'input_index'+str(j)+'512'+'.nii' in data_list:
            train_pair_tuple.append(('/home/yanxin/sorted_data_16/'+str(i)+'input_index'+str(j)+'512'+'.nii', '/home/yanxin/sorted_data_16/'+str(i)+'gt_index'+str(j)+'512'+'.nii'))
for i in range(120, 150):
    for j in [0,1,2]:
        if str(i)+'input_index'+str(j)+'512'+'.nii' in data_list:
            test_pair_tuple.append(('/home/yanxin/sorted_data_16/'+str(i)+'input_index'+str(j)+'512'+'.nii', '/home/yanxin/sorted_data_16/'+str(i)+'gt_index'+str(j)+'512'+'.nii'))
...
model =torch.load(str(520)+'U_model_size16_512.pkl')
# device_ids = [0, 1]
model = model.cuda()
# model = nn.DataParallel(model, device_ids=device_ids)# 载入模型的时候注视掉
...
for epoch in tqdm(range(1+520, num_epochs + 1+520)):
    ...
    train_loss_total_avg = train_loss_total / (len(train_dataset) / 8)
    model.eval()
    val_loss_total = 0.0
    num_steps = 0
    ...
    metric_mgr.reset()
    val_loss_total_avg = val_loss_total / (len(test_dataset) / 8)

    print('\nTrain loss: {:.4f}'.format(train_loss_total_avg))
    print('Val Loss: {:.4f}'.format(val_loss_total_avg))
    with open('score_Unet_size16_512.txt', 'a') as f:  # 'a'表示append,即在原来文件内容后继续写数据（不清楚原有数据）
        f.write('train:'+str(train_loss_total_avg)+'test'+str(val_loss_total_avg)+'\n')
    if epoch % 10 == 0:
        torch.save(model, str(epoch)+'U_model_size16_512.pkl')
        print('model saved')
